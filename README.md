...
Add to `.claude/settings.json` (project-level) or `~/.claude/settings.json` (global). Make sure to point to your local installation if running from source:
...
*Note: If installed globally via npm, you could use `npx -y x64dbg-mcp-server`.*
...
```bash
node E:/GitHub/x64dbg_mcp/server/build/index.js
```
